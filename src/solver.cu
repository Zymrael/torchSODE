...
template <typename scalar_t>
__global__ void
general_solver(method_t method, const Packed<scalar_t, 2> F_a, Packed<scalar_t, 1> x0_a, const Packed<scalar_t, 1> g_a, const scalar_t dt, int steps, size_t x0_size) {
...
torch::Tensor solve_cuda(torch::Tensor F, torch::Tensor x0, torch::Tensor g, float dt, int steps, std::string name){
    ...
    auto F_a = F.packed_accessor<scalar_t, 2>();
    auto x0_a = x0.packed_accessor<scalar_t, 1>();
    auto g_a = g.packed_accessor<scalar_t, 1>();
